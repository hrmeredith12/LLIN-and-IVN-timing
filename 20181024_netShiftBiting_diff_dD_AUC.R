...
cumulative_mal_prev_base <- AUC(time_base[1000:size(time_base,2)],infected_base[1000:size(time_base,2)], 
                                method = c("trapezoid","step","spline"),na.rm = FALSE)/365 # cumulative prevalence of malaria over 4 years
...
cumulative_mal_prev_net <- AUC(time_net[1000:size(time_net,2)],infected_net[1000:size(time_net,2)], method = c("trapezoid","step","spline"),na.rm = FALSE) # cumulative prevalence of malaria over 4 years

pltlist <- list()
x<-size(period_D_range, 2)
y<-size(dose_delay_range, 2)

someData <- rep(0, x*y)
cumulative_mal_ratio <- array(someData, c( x, y))
...
for (i in size(period_D_range, 2):1) {
  ...
  cumulative_mal_prev_net <- AUC(time_net[1000:size(time_net,2)],prevalence_ratio_net[1000:size(time_net,2)], 
      method = c("trapezoid","step","spline"),na.rm = FALSE)/365 # cumulative prevalence of malaria over 4 years
  ...
  cumulative_mal_prev_all <- AUC(time_all[1000:size(time_all,2)],prevalence_ratio_all[1000:size(time_all,2)], 
                                 method = c("trapezoid","step","spline"),na.rm = FALSE)/365 # cumulative prevalence of malaria over 4 years
  
  
  cumulative_mal_ratio[i,j] <- (cumulative_mal_prev_net - cumulative_mal_prev_all)/cumulative_mal_prev_net
